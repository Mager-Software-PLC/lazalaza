  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('Cannot connect') || errorMessage.includes('Failed to fetch')) {
        ...
        if (isProduction) {
          setError(`Cannot connect to backend. Please check VITE_API_URL_URL and rebuild. ${errorMessage.includes('Current API URL') ? errorMessage.split('Current API URL:')[1] : ''}`)
        } else {
